...
const secBooking = function () {
  let mysteryVariable = 0;

  return function () {
    mysteryVariable++;
    console.log(`${mysteryVariable} users`);
  };
};

const reservation = secBooking();
reservation();
reservation();
reservation();
...
const gamePlayers = function (n, wait) {
  const perTeam = n / 3;

  setTimeout(function () {
    console.log(`We are now starting with ${n} all players`);
    console.log(`There are 3 teams , each with ${perTeam} players`);
  }, wait * 1000);

  console.log(`Will start playing in ${wait} seconds`);
};
